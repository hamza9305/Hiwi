...
[ -z "$SAVE_PATH" ] && SAVE_PATH=results/theostereo_cyl_not_clipped_new_cyc
[ -z "$DATASET" ] && DATASET=/mnt/data_on_nvme2/home/haahm/Dataset/THEOStereo_Cyc_New_try
...
[ -z "$NUM_LOADER_WORKER" ] && NUM_LOADER_WORKER="16" # @haahm: increase or decrease the CPU dataloader threads as you wish
[ -z "$CHKPNT" ] && CHKPNT=checkpoint/sceneflow/sceneflow.tar
[ -z "$RESUME" ] && RESUME=results/theostereo_cyl_not_clipped_new_cyc
...
nice python finetune.py --maxdisp ${MAX_DISP} --with_spn --datapath "${DATASET}" --no_tensorboard --resize=1024x1024 \
    --save_path ${SAVE_PATH} --datatype theo+ --pretrained ${CHKPNT} --resume ${RESUME} --dontclip --num_test_imgs_to_store 10\
